...
public class Subwoofer : MonoBehaviour {
	...
	void Update () {
		if (Input.GetKeyDown("joystick 2 button 1") & cd<=0 & activity <=0){
			activity=duration;
			cd = countdown;
			Instantiate(cloud,  this.gameObject.transform.GetChild(0).position, Quaternion.Euler(0, 0, 60),  this.gameObject.transform.GetChild(0).transform);
			...
			Instantiate(cloud,  this.gameObject.transform.GetChild(2).position, Quaternion.Euler(0, 0, -120), this.gameObject.transform.GetChild(2).transform);
			Instantiate(cloud,  this.gameObject.transform.GetChild(3).position, Quaternion.Euler(0, 0, 180), this.gameObject.transform.GetChild(3).transform);
		}
		if (activity>0){
